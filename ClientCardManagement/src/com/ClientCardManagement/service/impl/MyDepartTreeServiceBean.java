...
import com.ClientCardManagement.service.MyDepartTreeService;
...
@Service
@Transactional
public class MyDepartTreeServiceBean implements MyDepartTreeService {
	@Resource
	...
	private String userid;//登陆userid，session中获取进行查询
	
	public DepartTree getCompanys() {
		ActionContext ctx = ActionContext.getContext();
		this.userid = String.valueOf((ctx.getSession().get("userid")));
		Query query = sessionFactory
				.getCurrentSession()
				.createQuery(
						"select tn.comjnam from  TrnNc as tn where tn.trnMid.crtusr = :uid and " +
						"tn.comjnam!=null and tn.comjnam !=:blank group by tn.comjnam ");
	...
	public DepartTree getDeparts(DepartTree trees) {
		...
		this.userid = String.valueOf((ctx.getSession().get("userid")));
		Query query;
		String hql = "select tn.postnam from TrnNc as tn where tn.trnMid.crtusr = :uid and tn.comjnam = :company " +
				"and tn.postnam != null and tn.postnam != :blank group by tn.postnam ";
		String blank = "";
		List result;
		String company;
		DepartNode node;
	...
	public DepartTree getNoDepartNodes(DepartTree trees) {
		...
		this.userid = String.valueOf((ctx.getSession().get("userid")));
		Query query;
		String hql = "select tn.id, tn.firstnam,tn.secdnam from TrnNc as tn where tn.trnMid.crtusr = :uid and " +
				"tn.comjnam = :company and tn.postnam = null or tn.comjnam = :company and tn.postnam = :blank order by tn.id";
		String blank = "";
		List result;
		String company;
		DepartNode node;
	...
	public DepartTree getLeafNodes(DepartTree trees) {
		...
		Query query;
		String hql = "select tn.id, tn.firstnam,tn.secdnam from TrnNc as tn where tn.trnMid.crtusr = :uid and " +
				"tn.comjnam = :company and tn.postnam = :postnam order by tn.id";
